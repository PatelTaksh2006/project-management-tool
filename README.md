...
- Add a `README` to both `backend/` and `frontend/` with more specific developer notes.
- Add example `.env.example` to `backend/` (do not commit secrets).
- Add tests and CI for both backend and frontend.
...
- commit the updated `README.md` for you.
